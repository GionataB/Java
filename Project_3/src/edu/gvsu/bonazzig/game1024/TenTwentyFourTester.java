...
import edu.gvsu.bonazzig.mathematics.Matrix;
...
public class TenTwentyFourTester {
    ...
    private static NumberGame gameLogic;
    ...
    public void randomValueShallBePlacedOnEmptySpot() {
        ...
        final int[][] M = new int[NROWS][NCOLS];//FIXME: without t
        Matrix.initializeValues(M);
        gameLogic.setValues(M);
        for (int k = 0; k < N; k++) {
...
//                    after[c.row][c.column] != 0);
    ...
    public void slideShallReturnFalseWhenBoardUnchanged()
    {
         ...
         */
    	gameLogic = new NumberGame();//FIXME: I put this.
    ...
    public void testSwipeUp()
    {
		...
		int[][] N = new int[6][6]; 
		Matrix.initializeValues(N);
		for(int i = 1; i < N.length; i++){
			for(int j = 0; j < N[i].length; j++){
				N[i][j] = N[i].length-j-1;
			}
		}
		for(int i = 1; i < N.length; i++){
			N[i][2] = 4;
		...
		N = Matrix.transpose(N);
		
		gameLogic.setValues(N);
		...
		System.out.println("After swipe:\n" + gameLogic.printTable());
		System.out.println("Expected:\n" + Matrix.toString(M));
		assertTrue(Matrix.equal(gameLogic.getBoard(), M));
    }
    ...
    //FIXME: to make this method work, comment out line 154 of NumberGame.java, the "placeRandomValue" line.
    @Test(timeout = 5000)
